...
async def tai_tep_vao_kho(
    ...
    """Nút "+" trong Kho tài liệu. Quản trị viên: lưu thẳng vào kho rồi hẹn lập
    chỉ mục. Người dùng đã đăng nhập: vào hàng chờ duyệt. Khách phải đăng nhập
    trước để quản trị viên biết tệp do ai gửi."""
    if x_rag_action != "upload-library":
        raise HTTPException(status_code=403, detail="Yêu cầu tải tệp không hợp lệ.")
    if tai_khoan.bat_khoa_quan_tri() and nguoi_dung_hien_tai(request) is None:
        raise HTTPException(status_code=401, detail="Hãy đăng nhập để gửi tài liệu vào kho.")
    do_dai = request.headers.get("content-length")
    if do_dai and do_dai.isdigit() and int(do_dai) > GIOI_HAN_BYTE:
        raise HTTPException(
            status_code=413,
